...
def get_comments(page):
    ...
    :return:
    """
    url = 'https://music.163.com/weapi/v1/resource/comments/R_SO_4_1313354324?csrf_token=' + str(page)
    response = requests.get(url=url, headers=headers)
    #将字符串转为json格式
    ...
    for item in items:
        ...
        with open('music_comments.csv','a',encoding='utf-8-sig') as f:
            f.wirte(user_name + ',' + user_id + ',' + user_age + ',' + user_city + ',' + user_introduce + ',' + comment + ',' + comment_id + ',' + praise + ',' + date + '\n')
            f.close()
